...
exports.handleCombat = handleCombat;
exports.canAttackAgain = canAttackAgain;
...
async function calculateDamage(attacker_id, defender_id) {
    const attackerStats = await characterService_1.default.getEquippedStats(attacker_id);
    const defenderStats = await characterService_1.default.getEquippedStats(defender_id);
...
async function handleCombat(attacker_id, defender_id, res) {
    try {
        const attacker = await databaseService_1.default.get("SELECT * FROM characters WHERE id = ?", [attacker_id]);
        if (!attacker)
            return res.status(404).json({ error: "Atacante no encontrado" });
        const defender = await databaseService_1.default.get("SELECT * FROM characters WHERE id = ?", [defender_id]);
        if (!defender)
        ...
        await databaseService_1.default.run(`UPDATE characters 
       SET currentGold = currentGold + ?, totalGold = totalGold + ?, currentXp = currentXp + ? 
       WHERE id = ?`, [goldGained, goldGained, xpGained, winner.id]);
        await databaseService_1.default.run(`UPDATE characters SET currentGold = currentGold - ? WHERE id = ?`, [lostGold, loser.id]);
        await saveBattle(attacker_id, defender_id, winner.id, goldGained, xpGained);
        // Notificar al jugador atacado
        if (sessionManager_1.connectedUsers.has(defender.id)) {
            (0, sessionManager_1.sendRealTimeNotification)(defender.id, `¡${attacker.name} te ha atacado!`);
        }
        else {
            await (0, messageService_1.sendMessage)(attacker_id, defender_id, "Has sido atacado", `El jugador ${attacker.name} te ha atacado.`);
        }
        return res.json({
...
async function canAttackAgain(attacker_id, defender_id) {
    try {
        const row = await databaseService_1.default.get(`SELECT last_attack FROM battles WHERE attacker_id = ? AND defender_id = ? 
       ORDER BY last_attack DESC LIMIT 1`, [attacker_id, defender_id]);
        if (!row)
            return true; // Nunca han peleado antes, puede atacar
...
async function saveBattle(attacker_id, defender_id, winner_id, goldWon, xpWon) {
    try {
        await databaseService_1.default.run(`INSERT INTO battles (attacker_id, defender_id, winner_id, gold_won, xp_won) 
       VALUES (?, ?, ?, ?, ?)`, [attacker_id, defender_id, winner_id, goldWon, xpWon]);
    }
    catch (error) {
