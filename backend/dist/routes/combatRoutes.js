...
router.post("/battle", authMiddleware_1.default, async (req, res) => {
    try {
        ...
        const allowed = await (0, combatService_1.canAttackAgain)(userId, defender_id);
        if (!allowed) {
            res.status(400).json({ error: "Debes esperar al menos 1 hora antes de atacar de nuevo." });
            return;
        }
        await (0, combatService_1.handleCombat)(userId, defender_id, res);
    }
    catch (error) {
...
router.get("/find-opponent", authMiddleware_1.default, async (req, res) => {
    try {
        ...
        const opponent = await new Promise((resolve, reject) => {
            database_1.db.get(`SELECT * FROM characters WHERE level BETWEEN ? AND ? AND user_id != ? ORDER BY RANDOM() LIMIT 1`, [minLevel, maxLevel, userId], (err, row) => {
                if (err)
                    return reject(err);
                resolve(row);
            });
...
router.post("/arena-battle", authMiddleware_1.default, async (req, res) => {
    try {
        ...
        const opponent = await new Promise((resolve, reject) => {
            database_1.db.get(`SELECT id FROM characters WHERE level BETWEEN (SELECT level FROM characters WHERE user_id = ?) - 2 
         AND (SELECT level FROM characters WHERE user_id = ?) + 2 AND user_id != ? ORDER BY RANDOM() LIMIT 1`, [userId, userId, userId], (err, row) => {
                if (err)
                    return reject(err);
                resolve(row);
            });
        ...
        await (0, combatService_1.handleCombat)(player.id, opponent.id, res);
    }
    catch (error) {
